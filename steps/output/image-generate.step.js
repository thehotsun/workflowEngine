...
class ImageGenerateStep extends BaseStep {
  ...
  async execute(context) {
    ...
    const seenImageUrls = new Set()  // 跨 slot 共享，避免重复下载同一张图
  ...
  async _generateCoverImage({ articleData, coverPrompt, assetsDir, imageNotes, photoSources, seenImageUrls }) {
    ...
    if (freePhoto.enabled) {
      try {
        ...
        if (result) {
          ...
          logger.info({ provider: result.meta.provider, query: result.meta.query, slot: 'cover' }, '✅ 封面图来源：免费图库')
          return result.path
        }
      } catch (err) {
    ...
    try {
      logger.info({ slot: 'cover' }, '🎨 封面图来源：AI 生图')
  ...
  async _generateInlineImage({ articleData, prompt, caption, slot, index, assetsDir, imageNotes, photoSources, seenImageUrls }) {
    const freePhoto = await this._freePhotoConfig()
    if (freePhoto.enabled) {
      try {
        ...
        if (result) {
          ...
          logger.info({ provider: result.meta.provider, query: result.meta.query, slot, index }, `✅ 插图 ${index} 来源：免费图库`)
          return result.path
        }
      } catch (err) {
    ...
    try {
      logger.info({ slot, index }, `🎨 插图 ${index} 来源：AI 生图`)
  ...
  async _fetchFreePhoto({ queries, slot, outputPath, minWidth, preferLandscape, freePhoto, seenImageUrls }) {
    const seenUrls = seenImageUrls || new Set()
